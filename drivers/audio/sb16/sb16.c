...
static void dsp_dma_setup(phys_bytes address, int count, int DmaMode) {
	...
	if(DspBits == 8) {   /* 8 bit sound */
		...
		pv_set(pvb[3], DMA16_ADDR, (u8_t)(address >> 1) & 0xFF);  /* Low_byte of address */
		pv_set(pvb[4], DMA16_ADDR, (u8_t)(address >> 9) & 0xFF);  /* High byte of address */
		pv_set(pvb[5], DMA16_PAGE, (u8_t)(address >> 16) & 0xFE); /* 128K page number */
